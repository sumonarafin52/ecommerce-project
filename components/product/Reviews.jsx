...
const Stars = ({ n, size = "w-4 h-4" }) => (
  <div className="flex gap-0.5">
    {[1, 2, 3, 4, 5].map((i) => (
      <svg key={i} className={`${size} ${i <= Math.round(n) ? "text-accent" : "text-zinc-600"}`} fill="currentColor" viewBox="0 0 20 20">
...
export default function Reviews({ productId }) {
  ...
  return (
    <section className="bg-primary-light border border-white/10 rounded-xl p-5 sm:p-6">
      <div className="flex flex-wrap items-center justify-between gap-3 mb-5">
        ...
        {reviews.length > 0 && (
          <div className="flex items-center gap-2">
            <Stars n={avg} />
            <span className="text-sm font-bold text-accent">{avg.toFixed(1)}</span>
      ...
      {loading ? (
        ...
        <p className="text-sm text-zinc-400 text-center py-8 border border-dashed border-white/10 rounded-lg">
          No reviews yet — verified buyers review dile ekhane dekhabe.
        </p>
      ) : (
        <div className="space-y-4">
          {reviews.map((r) => (
            <div key={r._id} className="bg-black/20 border border-white/5 rounded-lg p-4">
              <div className="flex items-center gap-3">
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-bold text-white">{r.user?.name || "Customer"}</p>
                  <p className="text-[11px] text-zinc-500">{formatDate(r.createdAt)} • Verified Purchase</p>
                </div>
                <Stars n={r.rating} />
              </div>
