...
export default function AdminLayout({ children }) {
  ...
  const sidebar = (
    <>
      <div className={`flex items-center gap-3 px-4 h-16 admin-border border-b ${collapsed ? "lg:justify-center lg:px-2" : ""}`}>
        <div className="w-9 h-9 rounded-lg bg-gradient-to-br from-accent to-orange-600 flex items-center justify-center shadow-lg shrink-0">
        ...
        {!collapsed && (
          <div className="flex-1 min-w-0">
            ...
            <p className="text-[10px] uppercase tracking-widest admin-text-muted mt-1">Admin Panel</p>
          </div>
        )}
        <button
          onClick={() => setCollapsed((c) => !c)}
          className="hidden lg:flex w-7 h-7 items-center justify-center rounded-md admin-border admin-text-secondary hover:text-accent transition-colors"
          aria-label="Toggle sidebar"
        >
          <Icon d={icons.chevron} className={`w-4 h-4 transition-transform ${collapsed ? "rotate-180" : ""}`} />
        </button>
        <button onClick={() => setOpen(false)} className="lg:hidden admin-text-secondary hover:text-accent text-xl" aria-label="Close menu">
          ✕
        </button>
      </div>
      ...
      <div className="flex-1 overflow-y-auto no-scrollbar px-3 py-4 space-y-5">
              ...
              {nav.map((item) =>
                item.children ? (
                  <div key={item.key}>
                    {collapsed ? (
                      ...
                      <>
                        ...
                        {prodOpen && (
                          <div className="ml-5 mt-1 space-y-0.5 admin-border border-l pl-3">
                            {item.children.map((c) => (
                              <Link
                                key={c.href}
                                href={c.href}
                  ...
                  <Link
                    ...
                    onClick={() => setOpen(false)}
                    title={item.label}
                    className={`relative flex items-center gap-3 rounded-md px-3 py-2.5 text-sm font-bold transition-all ${
        ...
        {!collapsed && pendingCount > 0 && can("orders") && (
          <Link
            href="/admin/orders"
            onClick={() => setOpen(false)}
            className="block rounded-lg bg-gradient-to-br from-accent/10 to-orange-500/10 border border-accent/20 p-3 hover:border-accent/40 transition-colors"
          >
        ...
        <div>
          ...
          <Link
            href="/"
            title="Online Store"
            className={`flex items-center gap-3 rounded-md px-3 py-2.5 text-sm font-bold admin-text-secondary hover:bg-gray-100 transition-all ${collapsed ? "lg:justify-center lg:px-0" : ""}`}
          >
            <Icon d={icons.store} />
  ...
  return (
    <div>
      <aside
        className={`admin-sidebar hidden lg:flex fixed inset-y-0 left-0 top-16 z-40 flex-col transition-all duration-300 ${
          collapsed ? "w-20" : "w-64"
        }`}
      >
        {sidebar}
      ...
      {open && (
        <div className="lg:hidden fixed inset-0 z-50">
          ...
          <aside className="absolute inset-y-0 left-0 top-16 w-72 admin-sidebar admin-border border-r flex flex-col">{sidebar}</aside>
        </div>
      )}
