...
export async function PUT(request, { params }) {
  try {
    ...
    if (body.addEvent?.status) {
      ...
      // keep the existing coarse orderStatus in sync for the statuses both
      // systems share — everything else (in_transit, out_for_delivery,
      // failed, returned) only lives in the granular shipment timeline so
      // nothing that already depends on orderStatus's 4-value enum breaks
